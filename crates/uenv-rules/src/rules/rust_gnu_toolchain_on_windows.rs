...
impl Rule for RustGnuToolchainOnWindows {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        if !crate::helpers::kind_is(env, "tauri") { return vec![]; }
        let active = crate::helpers::fact_str(env, "toolchain.rust", "active_toolchain");
        let Some(active) = active else { return vec![]; };
        if active.contains("-gnu") {
            vec![finding(self.id(), Severity::Warning, "Windows 上使用 GNU toolchain 构建 Tauri", desc).with_fix(fix_safety, fix_explain, fix_commands, fix_rollback)]
        } else { vec![] }
    }
}
...
mod tests {
    ...
    fn rule_triggers_and_silent() {
        ...
        crate::test_utils::with_detector(&mut env, "toolchain.rust", Layer::Toolchain,
            BTreeMap::from([("active_toolchain".to_string(), crate::test_utils::s("stable-x86_64-pc-windows-gnu"))]));
        ...
        crate::test_utils::with_detector(&mut env2, "toolchain.rust", Layer::Toolchain,
            BTreeMap::from([("active_toolchain".to_string(), crate::test_utils::s("stable-x86_64-pc-windows-msvc"))]));
