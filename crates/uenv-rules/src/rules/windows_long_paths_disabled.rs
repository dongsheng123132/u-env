...
impl Rule for WindowsLongPathsDisabled {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        let fix_commands: &[&str] = &["powershell -NoProfile -Command \"New-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\FileSystem' -Name LongPathsEnabled -Value 1 -PropertyType DWord -Force\""];
        let fix_rollback: &[&str] = &["powershell -NoProfile -Command \"Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\FileSystem' -Name LongPathsEnabled -Value 0\""];
        ...
        let sev = if is_node { Severity::Error } else { Severity::Warning };
        vec![finding(self.id(), sev, "长路径支持未开启", desc)
...
mod tests {
    ...
    fn rule_triggers_and_silent() {
        ...
        crate::test_utils::with_detector(&mut env, "windows.long-paths", Layer::Host,
        ...
        crate::test_utils::with_detector(&mut env2, "windows.long-paths", Layer::Host,
