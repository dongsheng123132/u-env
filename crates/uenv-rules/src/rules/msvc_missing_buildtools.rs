...
impl Rule for MsvcMissingBuildtools {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        let fix_commands: &[&str] = &["start https://visualstudio.microsoft.com/downloads/#build-tools-for-visual-studio-2022"];
        ...
        match has_cpp {
            Some(true) => vec![],
            _ => vec![finding(self.id(), Severity::Error, "缺少 MSVC 构建工具", desc).with_fix(fix_safety, fix_explain, fix_commands, fix_rollback)],
        }
    }
}
...
mod tests {
    ...
    fn rule_triggers_and_silent() {
        ...
        crate::test_utils::with_detector(&mut env, "toolchain.msvc", Layer::Toolchain,
        ...
        crate::test_utils::with_detector(&mut env2, "toolchain.msvc", Layer::Toolchain,
