...
impl Rule for NodePackageManagerMismatch {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        let fix_commands: &[&str] = &["echo \"删除多余锁文件后重新安装：pnpm install（以声明 pnpm 为例）\""];
        let fix_rollback: &[&str] = &["git checkout -- package-lock.json yarn.lock pnpm-lock.yaml 2>NUL"];
        let pm = crate::helpers::fact_str(env, "project.manifests", "package_manager");
        let lockfiles = crate::helpers::lockfile_names(env);
        let Some(pm) = pm else { return vec![]; };
        ...
        if !lockfiles.is_empty() && !lockfiles.iter().any(|f| f == expected) {
            vec![finding(self.id(), Severity::Warning, "包管理器与锁文件不一致", desc).with_fix(fix_safety, fix_explain, fix_commands, fix_rollback)]
        } else { vec![] }
    }
}
...
mod tests {
    ...
    fn rule_triggers_and_silent() {
        ...
        crate::test_utils::with_detector(&mut env, "project.manifests", Layer::Project,
        ...
        crate::test_utils::with_detector(&mut env, "project.lockfiles", Layer::Project,
            BTreeMap::from([("lockfiles".to_string(), FactValue::Map(BTreeMap::from([
                ("package-lock.json".to_string(), crate::test_utils::s("abc")),
            ])))]));
        ...
        crate::test_utils::with_detector(&mut env2, "project.manifests", Layer::Project,
            BTreeMap::from([("package_manager".to_string(), crate::test_utils::s("pnpm"))]));
        crate::test_utils::with_detector(&mut env2, "project.lockfiles", Layer::Project,
            BTreeMap::from([("lockfiles".to_string(), FactValue::Map(BTreeMap::from([
                ("pnpm-lock.yaml".to_string(), crate::test_utils::s("abc")),
            ])))]));
