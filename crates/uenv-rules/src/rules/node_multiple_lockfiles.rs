...
impl Rule for NodeMultipleLockfiles {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        let fix_explain = "删除不用的锁文件，只留当前包管理器对应的一份（如保留 pnpm-lock.yaml，删除 package-lock.json 和 yarn.lock；误删可 git checkout -- <文件名> 找回）";
        let n = crate::helpers::lockfile_names(env).len();
        if n >= 2 {
            vec![
                finding(self.id(), Severity::Warning, "存在多种锁文件", desc)
