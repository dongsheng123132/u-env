...
impl Rule for SecurityDefenderScansProject {
    ...
    fn evaluate(&self, env: &Environment) -> Vec<Finding> {
        ...
        let fix_commands: &[&str] = &["powershell -NoProfile -Command \"Add-MpPreference -ExclusionPath '<项目路径>'\""];
        let fix_rollback: &[&str] = &["powershell -NoProfile -Command \"Remove-MpPreference -ExclusionPath '<项目路径>'\""];
        let rt = crate::helpers::fact_bool(env, "security.defender", "realtime_enabled");
        let Some(true) = rt else { return vec![]; };
        let covers = crate::helpers::fact_bool(env, "security.defender", "exclusion_covers_project");
        match covers {
            Some(true) => vec![],
            _ => vec![finding(self.id(), Severity::Info, "Defender 实时保护未排除项目目录", desc).with_fix(fix_safety, fix_explain, fix_commands, fix_rollback)],
        }
    }
}
...
mod tests {
    ...
    fn rule_triggers_and_silent() {
        ...
        crate::test_utils::with_detector(&mut env, "security.defender", Layer::Host,
            BTreeMap::from([
                ("realtime_enabled".to_string(), crate::test_utils::b(true)),
                ("exclusion_covers_project".to_string(), crate::test_utils::b(false)),
            ]));
        assert_eq!(SecurityDefenderScansProject.evaluate(&env).len(), 1);

        let mut env2 = crate::test_utils::empty_env();
        crate::test_utils::with_detector(&mut env2, "security.defender", Layer::Host,
            BTreeMap::from([
                ("realtime_enabled".to_string(), crate::test_utils::b(true)),
                ("exclusion_covers_project".to_string(), crate::test_utils::b(true)),
            ]));
