---
name: uking-pdf
description: 导出 PDF。Markdown / HTML 走系统自带的 Edge/Chrome（零安装、0.5 秒、中文可搜索），Word/Excel/PPT 走 LibreOffice 保版式。当用户说「导出 PDF」「转成 PDF」「做成 PDF 发给客户」「PDF 版本」「不要让对方能改」时使用。
---
...
## 核心用法
> 脚本随本 skill 分发，`<本 SKILL.md 同目录>` 指本 SKILL.md 所在目录（插件里即 `skills/uking-pdf/`）。

```bash
node <本 SKILL.md 同目录>/scripts/to-pdf.mjs --check --json      # 先看这台机器有哪条引擎
node <本 SKILL.md 同目录>/scripts/to-pdf.mjs 报告.md --json       # 零安装路径
node <本 SKILL.md 同目录>/scripts/to-pdf.mjs 合同.docx --json     # 需要 LibreOffice
node <本 SKILL.md 同目录>/scripts/to-pdf.mjs 报表.xlsx --out D:/发货/报表.pdf --json
node <本 SKILL.md 同目录>/scripts/to-pdf.mjs 页面.html --engine chromium --json
```
