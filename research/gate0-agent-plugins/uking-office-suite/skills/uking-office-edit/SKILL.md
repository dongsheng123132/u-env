...
description: 在用户**已有的** Word / PPT / Excel 上改文字，而不丢格式。当用户说「帮我把这份合同里的甲方改成…」「这份方案里的年份换一下」「把表格里所有的 X 替换成 Y」，或者给了一份 .docx/.pptx/.xlsx 并要求修改其中的文字时使用。**改现有文件用这个，从零写新文件用 uking-docx / uking-ppt / uking-xlsx。**
---
